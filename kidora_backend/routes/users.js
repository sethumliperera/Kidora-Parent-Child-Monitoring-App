...
async function ensureParentFcmColumn() {
  try {
    await db.query(
    ...
    if (err.errno !== 1060 && !String(err.message || "").includes("Duplicate column")) {
      console.error("ensureParentFcmColumn:", err.message);
    }
  }
}
...
// ===============================
// Parent device FCM token (push when app is closed)
// ===============================
router.post("/me/fcm-token", verifyToken, async (req, res) => {
  try {
    await ensureParentFcmColumn();
    ...
    console.log(`Parent FCM token saved for user id=${req.user.id}`);
    res.json({ message: "ok" });
  } catch (err) {
    console.error("parent fcm-token:", err);
    res.status(500).json({ message: err.message });
  }
});
